...
export default function ApplicationForm() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      howDidYouKnowSpecification: "", // Add the new field
      motivation: "",
      linkedInProfile: "",
      githubProfile: "",
      nationalId: "",
    ...
    if (savedFormData) {
      try {
        ...
        if (parsedData.interviewDate) {
          parsedData.interviewDate = parsedData.interviewDate ? new Date(parsedData.interviewDate) : null
        }
        if (parsedData.decisionDate) {
          parsedData.decisionDate = parsedData.decisionDate ? new Date(parsedData.decisionDate) : null
        }
      ...
      <CardContent className="pt-6 bg-white">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            {formError && <p className="text-sm text-red-600 text-center mt-2">{formError}</p>}
          </form>
        </Form>
      </CardContent>
    </Card>
